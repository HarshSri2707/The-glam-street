...
const Navigation = ({ data }) => {
  ...
  return (
    <nav 
      className={`fixed top-0 inset-x-0 z-[150] transition-all duration-500 ease-in-out ${
        isScrolled || isMobileMenuOpen 
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex justify-between items-center h-12">
          ...
          {/* Logo - Stays visible and interactive */}
          <a href="/" className="relative z-[200]">
            <h1 className={`font-serif text-xl md:text-2xl font-bold tracking-tighter transition-colors duration-300 ${
              !isScrolled && !isMobileMenuOpen ? 'text-white' : 'text-slate-900'
            }`}>
              {data.logo}<span className="text-pink-500">.</span>
          ...
          <div className="flex items-center gap-4 relative z-[200]">
            {/* Search Icon */}
            <button className={`p-1 transition-colors ${!isScrolled && !isMobileMenuOpen ? 'text-white' : 'text-slate-700'}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            </button>

            {/* Mobile Toggle - Enhanced Logic */}
            <button
              className="lg:hidden p-2 focus:outline-none"
              onClick={toggleMenu}
            >
              <div className="w-6 flex flex-col items-end gap-1.5">
                <span className={`h-0.5 transition-all duration-300 rounded-full ${
                  isMobileMenuOpen 
                    ? 'w-6 rotate-45 translate-y-2 bg-slate-900' 
                    : `w-6 ${!isScrolled ? 'bg-white' : 'bg-slate-900'}`
                }`} />
                <span className={`h-0.5 transition-all duration-300 rounded-full ${
                  isMobileMenuOpen 
                    ? 'opacity-0' 
                    : `w-4 ${!isScrolled ? 'bg-white' : 'bg-slate-900'}`
                }`} />
                <span className={`h-0.5 transition-all duration-300 rounded-full ${
                  isMobileMenuOpen 
                    ? 'w-6 -rotate-45 -translate-y-2 bg-slate-900' 
                    : `w-5 ${!isScrolled ? 'bg-white' : 'bg-slate-900'}`
                }`} />
              </div>
            </button>

            {/* Desktop CTA */}
            <a href={data.cta.href} className="hidden md:block px-6 py-2.5 bg-pink-600 text-white text-[10px] font-bold uppercase tracking-widest rounded-full hover:shadow-pink-500/20 shadow-lg">
              {data.cta.label}
            </a>
          </div>
        </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ y: '-100%' }}
            transition={{ type: 'spring', damping: 25, stiffness: 200 }}
            className="fixed inset-0 bg-white z-[180] lg:hidden flex flex-col overflow-y-auto"
          >
            {/* Background Decor */}
            <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-br from-pink-50/40 via-white to-transparent -z-10" />
            ...
            <div className="flex flex-col h-full px-8 pt-28 pb-10">
                ...
                {data.menu.map((item, index) => (
                  ...
                  >
                    <a
                      href={item.href}
                      onClick={toggleMenu}
                      className="block py-4 text-2xl font-serif italic text-slate-800 border-b border-slate-50 hover:text-pink-600"
                    >
                      {item.label}
                    </a>
                  </motion.div>
              ...
              >
                <a 
                  href={data.cta.href} 
                  onClick={toggleMenu}
                  className="block w-full py-4 bg-slate-900 text-white text-center rounded-xl font-bold uppercase tracking-widest text-[10px] shadow-2xl shadow-slate-200"
                >
                  {data.cta.label}
                </a>
                ...
                <div className="flex justify-center gap-8 mt-8">
                  ...
                  <span className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Pinterest</span>
                </div>
              </motion.div>
            </div>
          </motion.div>
